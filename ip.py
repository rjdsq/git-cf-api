...
def get(url):
    try:
        ...
        res = requests.get(url, timeout=12, headers=hd)
        return res.text
    except: return ""
...
def main():
    ...
    for api in API_LIST:
        ...
        for line in txt.splitlines():
            ...
            processed_line = rebuild_line(s)
            
            if addr:
                if addr in seen_addresses: continue
                seen_addresses.add(addr)
                if IPV6_REG.search(addr):
                    list_ipv6.append(processed_line)
                else:
                    list_ipv4.append(processed_line)
            else:
                host = base.split(":")[0].lstrip("[").rstrip("]")
                if host in seen_addresses: continue
                seen_addresses.add(host)
                list_domains.append(processed_line)

    for raw_item in DOMAIN_LIST:
        host, rebuilt = process_domain_item(raw_item)
        if host not in seen_addresses:
            seen_addresses.add(host)
            list_domains.append(rebuilt)

    list_domains.sort(key=lambda x: len(x.split("#")[0]))

    def ip_sort_key(line):
        base, remark = line.split("#", 1)
        country = remark.split("|")[0].strip() if "|" in remark else remark
        return (country, len(base))

    list_ipv4.sort(key=ip_sort_key)
    list_ipv6.sort(key=ip_sort_key)

    final_output = list_domains + list_ipv4 + list_ipv6
